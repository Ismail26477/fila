  ...
  return (
    <header
      className={cn(
        ...
        solid
          ? "border-b border-primary-foreground/10 bg-navy-deep"
      ...
      <div className="container-x flex h-18 items-center justify-between gap-4 py-3 md:h-20">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 lg:flex">
          {nav.map((item) => (
            ...
            >
              {item.label}
              <span className="absolute inset-x-4 -bottom-0.5 h-px scale-x-0 bg-blue-muted transition-transform duration-300 group-hover:scale-x-100 group-data-[status=active]:scale-x-100" />
            </Link>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="overflow-hidden border-t border-primary-foreground/10 bg-navy-deep lg:hidden"
          >
