...
export function HeroSlider() {
  ...
  return (
    <section
      ...
      className="relative isolate h-[100svh] overflow-hidden bg-navy-deep text-primary-foreground"
    >
        ...
        <div className="container-x w-full">
          <div className="max-w-2xl pt-24 pb-24 md:pt-20">
            ...
            <h1 className="mt-5 text-3xl leading-[1.08] font-extrabold sm:text-5xl lg:text-6xl">
              {before}
              <span className="text-blue-muted">{slide.highlight}</span>
              {after}
            </h1>
          ...
          {slides.map((s, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all ${
                i === index ? "w-10 bg-blue-muted" : "w-4 bg-primary-foreground/40"
              }`}
            />
          ))}
        </div>
