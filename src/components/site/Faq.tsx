    ...
    <section className="section-y bg-background">
      <div className="container-x grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
        <Reveal>
          ...
          <h2 className="mt-5 text-2xl leading-tight font-extrabold text-navy sm:text-3xl">
            ...
            {faqs.map((item) => (
              <li key={item.q}>
                <details className="group py-5">
                  <summary className="flex cursor-pointer list-none items-start justify-between gap-4 font-display text-base font-bold text-navy">
                    <span className="min-w-0">{item.q}</span>
                    <span
                      className="mt-1 shrink-0 text-plum transition-transform duration-300 group-open:rotate-45"
                      aria-hidden="true"
                    >
                      +
                    </span>
                  </summary>
                  <p className="mt-3 pr-8 text-sm leading-relaxed text-slate-blue">{item.a}</p>
                </details>
              </li>
            ))}
          </ul>
